...
router.post("/add", async (req, res) => {
    ...
    if(!req.body.qntBairros){
        res.status(404).json({message: "Quantidade de bairros da requisição cidade está vazia."});
        return;
    };
    ...
    if(!req.body.dataAniversario){
        res.status(404).json({message: "Data de aniversário da requisição cidade está vazia."});
        return;
    };
...
router.put("/update/:id", async (req, res) => {
    ...
    if (req.params.id == null){
        ...
        return;
    }else if (!req.body.qntBairros){
        res.status(404).json({message:"A quantidade de bairros da cidade não foi informada."})
        return;
    }else if (!req.body.populacao){
        res.status(404).json({message: "A populacao da cidade não foi informada."})
        return;
    }else if (!req.body.dataAniversario){
        res.status(404).json({message: "A data de aniversário da cidade não foi informada."})
        return;
    }
