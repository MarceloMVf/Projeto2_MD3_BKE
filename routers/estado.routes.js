...
router.post("/add", async (req, res) => {
    ...
    if(!req.body.salarioMin){
        res.status(404).json({message: "Salário mínimo da requisição estado está vazia."});
        return;
    };
...
router.put("/update/:id", async (req, res) => {
    ...
    if (req.params.id == null){
        ...
        res.status(404).json({message: "O salário mínimo do estado não foi informado."})
        return;
    }
